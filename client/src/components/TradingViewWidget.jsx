...
function TradingViewWidget() {
  ...
  const [height, setHeight] = useState(() => (typeof window !== 'undefined' && window.innerWidth < 640 ? 140 : 360));
  const [loadFailed, setLoadFailed] = useState(false);

  useEffect(() => {
    const handleResize = () => {
      setHeight(window.innerWidth < 640 ? 140 : 360);
    };
  ...
  useEffect(() => {
    ...
    script.async = true;
    script.onload = () => {
      console.log('TradingView market-overview embed script loaded');
      setLoadFailed(false);
    };
    script.onerror = (err) => {
      console.error('TradingView market-overview failed to load', err);
      setLoadFailed(true);
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        marginBottom: '24px',
        minHeight: height < 400 ? '120px' : '220px',
        maxHeight: 'none',
        overflow: 'hidden',
      }}
    >
