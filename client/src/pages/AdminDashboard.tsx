                ...
                <tbody>
                  {(Array.isArray(transactions) ? transactions : []).map((tx: any) => (
                    <tr key={tx._id || tx.id} className="border-b hover:bg-gray-100">
                      ...
                      <td className="text-left">{tx.userId?.fullName || tx.userId?.name || 'N/A'}</td>
                ...
                <tbody>
                  {(Array.isArray(investments) ? investments : []).map((inv: any) => (
                    <tr key={inv._id || inv.id} className="border-b hover:bg-gray-100">
                      ...
                      <td className="text-left">{inv.userId?.fullName || inv.userId?.name || 'N/A'}</td>
                ...
                <tbody>
                  {(Array.isArray(payments) ? payments : []).map((pay: any) => (
                    <tr key={pay._id || pay.id} className="border-b hover:bg-gray-100">
                      <td className="text-left">{pay.userId?.email || pay.user?.email || 'N/A'}</td>
                      <td className="text-left">{pay.userId?.fullName || pay.user?.fullName || 'N/A'}</td>
                      ...
                      <td className="text-left">
                        {pay.status === 'awaiting_confirmation' || pay.status === 'pending' || pay.status === 'Pending' ? (
                          <>
                            <button
